...
aldrin |>
  group_by(depth) |>
  summarise(statistic = shapiro.test(concentration)$statistic,
...
aldrin |>
  group_by(depth) |>
  summarise(sd_concentration = sd(concentration)) |>
  summarise(ratio_max_min_sd = max(sd_concentration) / min(sd_concentration))
```

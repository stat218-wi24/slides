...
- In general, we don't know the true proportion $p$, so we substitute some value to check conditions and estimate the standard error. 

  - __For confidence intervals__, the sample proportion $\hat{p}$ is used to check
the success-failure condition and compute the standard error. 
...
:::
  ...
  - __For hypothesis tests__, typically the null value - that is, the proportion claimed in the null hypothesis -  is used in place of $p$.
...
$$
\hat{p}_1 - \hat{p}_2 \pm z^{\ast} \times \sqrt{\frac{p_1(1-p_1)}{n_1} + \frac{p_2(1-p_2)}{n_2}}
$$

...
- reviewed the basic concepts of probability 
...
- describe the term "sampling distribution".  
...
- elaborate data visualization techniques in R.
- review the Midterm 1 topics.


## Introduction
::: {style="font-size: 30px"}
- In data analysis, the essential aim is to separate actual biological features from those that might be the result of random chance.

- Generally we obtain only a single random sample, which comes from a very large population.
  - Today, we will see what would happen if we select 
    - a single random sample with the different sample sizes or
    - infinitely many samples with replacement from the same population.
  - Then, we will find out what we will do if we do not have any information regarding population parameters.
    - How can we estimate population parameters (e.g., population mean).
...
__Revisiting Fish Length Example__
...
We will draw multiple samples of three different sizes and simulate data to see how it affects the variability of the sample proportions for different sample sizes. 
...
- Number of Candies is 10  
  ...
  - Fix the number of candies as 10  
  - Choose statistics as Number of Orange  
  - See first what will happen if you draw only 1 sample.  
  - Try this 4-5 times.
  - See what will happen if you increase the number of samples to 1,000.  
...
- Number of Candies is 10, 25, 1000
  - Set the probability of orange is 0.5
  - Fix the number of candies as 10
  - __Choose statistics__  as proportion of orange
  - See first what will happen if you have only 1-3 samples.
  - See what will happen if you increase the number of samples to 1,000.
:::
:::
:::
...
We can also use these take away messages to the fish length variable.
